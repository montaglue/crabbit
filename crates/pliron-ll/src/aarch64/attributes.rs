...
impl Parsable for RegisterAttr {
    ...
    fn parse<'a>(
        ...
        let Some(register) = Register::parse(&text) else {
            input_err!(loc, "invalid AArch64 register `{text}`")?
        };
        Ok((RegisterAttr(register), commit))
    }
}

/// An AArch64 condition code: the 4-bit `cond` field tested by `b.cond` and
/// `cset`. Discriminants are the hardware encoding. `al`/`nv` (0xe/0xf) are
/// deliberately unrepresentable: they are not conditional (unconditional
/// branches are `b`), and excluding them makes [ConditionCode::invert] total.
...
const CONDITION_NAMES: [&str; 14] = [
    "eq", "ne", "hs", "lo", "mi", "pl", "vs", "vc", "hi", "ls", "ge", "lt", "gt", "le",
];

impl ConditionCode {
    pub const ALL: [ConditionCode; 14] = [
        Self::Eq,
        Self::Ne,
        Self::Hs,
        Self::Lo,
    ...
    /// The 4-bit `cond` field of `B.cond` and the condition-select
    /// instructions.
    pub fn encoding(self) -> u8 {
        self as u8
    }

    /// The condition that holds exactly when `self` does not (eq<->ne,
    /// hs<->lo, ge<->lt, ...).
...
impl Parsable for ConditionCodeAttr {
    ...
    fn parse<'a>(
        ...
        let Some(condition) = ConditionCode::parse(&text) else {
            input_err!(loc, "invalid AArch64 condition code `{text}`")?
        };
        Ok((ConditionCodeAttr(condition), commit))
    }
}

/// An argument or result location assigned by the Darwin AArch64 ABI.
...
pub enum AbiLocation {
    ...
    /// The caller passes a result pointer in this register (`x8`).
    IndirectResult { reg: Register },
...
/// Where a function's arguments and result live under the Darwin AArch64 ABI.
...
#[def_attribute("aarch64.function_abi")]
#[derive(PartialEq, Eq, Clone, Debug, Hash)]
...
impl Verify for FunctionAbiAttr {
    fn verify(&self, _ctx: &Context) -> STAIRResult<()> {
        if self
            ...
            .args
            .iter()
            .any(|location| matches!(location, AbiLocation::IndirectResult { .. }))
        {
...
impl Printable for AbiLocation {
    fn fmt(
        ...
        match self {
            ...
            Self::IndirectResult { reg } => write!(f, "sret:{reg}"),
...
/// `void`, `sret:<reg>`, `stack:<offset>`, `<reg>`, or `<reg>:<reg>`.
fn parse_abi_location<'a>(
    ...
    match text.as_str() {
        ...
        "sret" => {
            let (reg, commit) = token(':')
                .with(many1::<String, _, _>(satisfy(|c: char| {
                    c.is_ascii_alphanumeric()
                })))
                .parse_stream(state_stream)
                .into_result()?;
            let Some(reg) = Register::parse(&reg) else {
                input_err!(loc, "invalid AArch64 ABI location register `{reg}`")?
            };
            Ok((AbiLocation::IndirectResult { reg }, commit))
        }
        "stack" => {
        ...
        first => {
            let Some(first) = Register::parse(first) else {
                input_err!(loc, "invalid AArch64 ABI location `{text}`")?
            };
            ...
            let Some(second) = Register::parse(&second) else {
                input_err!(loc, "invalid AArch64 ABI location register `{second}`")?
            };
...
/// [Printable] emits, e.g. `call26:4:puts`.
...
mod tests {
    ...
    fn function_abi_attr_round_trips_through_text() {
        ...
        let attr = FunctionAbiAttr(FunctionAbi {
            args: vec![
                AbiLocation::Gpr(Register::gpr(0)),
                AbiLocation::GprPair(Register::gpr(1), Register::gpr(2)),
                AbiLocation::Stack(16),
                AbiLocation::Void,
            ],
            result: AbiLocation::IndirectResult { reg: X8 },
        });
        let text = attr.disp(&ctx).to_string();
        assert_eq!(text, "([x0, x1:x2, stack:16, void], sret:x8)");
    ...
    fn fixups_attr_round_trips_through_text() {
        ...
        let text = attr.disp(&ctx).to_string();
        assert_eq!(text, "[call26:4:puts, call26:16:_my.helper$1]");
    ...
    fn function_abi_attr_parses_empty_args() {
        ...
        let attr = FunctionAbiAttr(FunctionAbi {
            args: vec![],
            result: AbiLocation::Gpr(Register::gpr(0)),
        });
        let text = attr.disp(&ctx).to_string();
        assert_eq!(text, "([], x0)");
