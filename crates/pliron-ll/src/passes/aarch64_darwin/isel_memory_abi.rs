...
pub(super) fn result_location_for_type(
    ...
    if is_aggregate_ty(ctx, ty) {
        ...
        if size <= 8 {
            ...
            Ok(ResultLocation::IndirectX8)
        }
    } else {
        Ok(ResultLocation::ScalarX0)
    }
}
...
pub(super) fn emit_return_value(
    ...
    match result {
        ...
        AbiLocation::Gpr(reg) => {
            ...
            aarch64_ops::mov(ctx, reg, src).insert_at_back(entry, ctx);
            Ok(())
        }
        AbiLocation::GprPair(lo_reg, hi_reg) => {
            ...
            if is_128_bit_integer(ctx, value.get_type(ctx)) {
                ...
                aarch64_ops::mov(ctx, lo_reg, lo).insert_at_back(entry, ctx);
                aarch64_ops::mov(ctx, hi_reg, hi).insert_at_back(entry, ctx);
                Ok(())
            } else {
...
fn load_direct_aggregate_from_gprs(
    ...
    if !is_aggregate_ty(ctx, ty) {
        if is_128_bit_integer(ctx, ty) {
            ...
            aarch64_ops::mov(ctx, lo, Register::gpr(*next_reg as u8)).insert_at_back(entry, ctx);
            *next_reg += 1;
            let hi = fresh_vreg(next_vreg);
            aarch64_ops::mov(ctx, hi, Register::gpr(*next_reg as u8)).insert_at_back(entry, ctx);
            *next_reg += 1;
        ...
        let dst = fresh_vreg(next_vreg);
        aarch64_ops::mov(ctx, dst, Register::gpr(*next_reg as u8)).insert_at_back(entry, ctx);
        *next_reg += 1;
    ...
    if count == 1 && stack_size_of(ctx, ty)? <= 8 {
        ...
        aarch64_ops::mov(ctx, base, Register::gpr(*next_reg as u8)).insert_at_back(entry, ctx);
        *next_reg += 1;
...
fn emit_direct_aggregate_to_gprs(
    ...
    if !is_aggregate_ty(ctx, ty) {
        if is_128_bit_integer(ctx, ty) {
            ...
            aarch64_ops::mov(ctx, Register::gpr(*next_reg as u8), lo).insert_at_back(entry, ctx);
            *next_reg += 1;
            aarch64_ops::mov(ctx, Register::gpr(*next_reg as u8), hi).insert_at_back(entry, ctx);
            *next_reg += 1;
            return Ok(());
        }
        if *next_reg >= 2 {
        ...
        aarch64_ops::mov(ctx, Register::gpr(*next_reg as u8), src).insert_at_back(entry, ctx);
        *next_reg += 1;
        return Ok(());
    }
    ...
    if stack_size_of(ctx, ty)? <= 8 {
        ...
        aarch64_ops::mov(ctx, Register::gpr(*next_reg as u8), packed).insert_at_back(entry, ctx);
        *next_reg += 1;
        return Ok(());
    }
    ...
    if stack_size_of(ctx, ty)? <= 16 {
        ...
        for (reg, half) in [acc.0, acc.1].into_iter().enumerate() {
            ...
            aarch64_ops::mov(ctx, Register::gpr((*next_reg + reg) as u8), src).insert_at_back(entry, ctx);
        }
        *next_reg += 2;
        return Ok(());
    }
...
fn store_stack_value(
    ...
    if is_stack_scalar_ty(ctx, ty) {
        ...
        if is_128_bit_integer(ctx, ty) {
            ...
            aarch64_ops::str_sp_offset(ctx, lo, offset).insert_at_back(entry, ctx);
            aarch64_ops::str_sp_offset(ctx, hi, offset + 8).insert_at_back(entry, ctx);
            return Ok(());
        }
        let src = materialize(ctx, entry, value, next_vreg, "store value")?;
        aarch64_ops::str_sp_offset_sized(ctx, store_sp_opcode(ctx, ty)?, src, offset)
            .insert_at_back(entry, ctx);
        return Ok(());
    }
    ...
    if is_stack_scalar_ty(ctx, ty) {
        ...
        if is_128_bit_integer(ctx, ty) {
            ...
            aarch64_ops::str_reg_offset_sized(
                ctx,
                aarch64_ops::StrRegOffsetOp::OPCODE,
                lo,
                base,
                offset,
            )
        ...
        let src = materialize(ctx, entry, value, next_vreg, "store value")?;
        aarch64_ops::str_reg_offset_sized(ctx, store_reg_opcode(ctx, ty)?, src, base, offset)
            .insert_at_back(entry, ctx);
        return Ok(());
    }
