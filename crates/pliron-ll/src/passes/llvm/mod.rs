...
/// licm, gvn — each self-disables via [midend_gate]) and a final
/// clean-up round.
...
pub fn add_llvm_midend_passes(passes: &mut Passes, _profile: &TargetProfile) {
    // _profile: first consumer (sink's divergence rule) arrives with the
    // backward-dataflow round; the signature is fixed here so callers and
    // later rounds need no churn.
