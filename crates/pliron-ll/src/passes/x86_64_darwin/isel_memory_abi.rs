...
pub(super) fn lower_gep(
    ...
    for index in indices {
        ...
        if let Some(value) = index_const {
            ...
            let scaled = if element_size == 1 {
                ...
                x86_64_ops::binary(ctx, x86_64_ops::MulOp::OPCODE, scaled, reg, scale)
                    .insert_at_back(entry, ctx);
                scaled
            };
            dynamic_offset = Some(if let Some(acc) = dynamic_offset {
                let dst = fresh_vreg(next_vreg);
                x86_64_ops::binary(ctx, x86_64_ops::AddOp::OPCODE, dst, acc, scaled)
                    .insert_at_back(entry, ctx);
                dst
            } else {
                scaled
    ...
    let base = if let Some(offset) = dynamic_offset {
        let dst = fresh_vreg(next_vreg);
        x86_64_ops::binary(ctx, x86_64_ops::AddOp::OPCODE, dst, base, offset)
            .insert_at_back(entry, ctx);
        dst
    } else {
        base
...
pub(super) fn emit_return_value(
    ...
    match result {
        ...
        AbiLocation::IndirectResult => {
            ...
            let dst = fresh_vreg(next_vreg);
            x86_64_ops::ldr_sp_offset(ctx, dst, sret_result_slot.offset)
                .insert_at_back(entry, ctx);
...
fn unpack_aggregate_from_reg(
    ...
    if is_stack_scalar_ty(ctx, ty) {
        ...
        if size < 8 {
            ...
            x86_64_ops::binary(ctx, x86_64_ops::AndOp::OPCODE, masked, value, mask)
                .insert_at_back(entry, ctx);
            value = masked;
        }
...
fn pack_aggregate_to_reg(
    ...
    if is_stack_scalar_ty(ctx, ty) {
        ...
        if scalar_size_of(ctx, ty)? < 8 {
            ...
            let masked = fresh_vreg(next_vreg);
            x86_64_ops::binary(ctx, x86_64_ops::AndOp::OPCODE, masked, reg, mask)
                .insert_at_back(entry, ctx);
            reg = masked;
        }
        if byte_offset > 0 {
            ...
            materialize_u64_immediate(ctx, entry, shift, byte_offset * 8);
            let shifted = fresh_vreg(next_vreg);
            x86_64_ops::binary(ctx, x86_64_ops::ShlOp::OPCODE, shifted, reg, shift)
                .insert_at_back(entry, ctx);
            reg = shifted;
        }
        return Ok(reg);
...
pub(super) fn load_stack_value(
    ...
    if is_stack_scalar_ty(ctx, ty) {
        if is_128_bit_integer(ctx, ty) {
            let lo = fresh_vreg(next_vreg);
            x86_64_ops::ldr_sp_offset(ctx, lo, offset).insert_at_back(entry, ctx);
            let hi = fresh_vreg(next_vreg);
            x86_64_ops::ldr_sp_offset(ctx, hi, offset + 8).insert_at_back(entry, ctx);
            return Ok(LoweredValue::RegPair(lo, hi));
        }
        let dst = fresh_vreg(next_vreg);
        x86_64_ops::ldr_sp_offset_sized(ctx, load_sp_opcode(ctx, ty)?, dst, offset)
            .insert_at_back(entry, ctx);
