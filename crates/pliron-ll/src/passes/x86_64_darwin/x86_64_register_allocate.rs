...
use crate::{
    ...
    dialects::{
        x86_64::{
            ...
            registers::{self, Register, RegisterClass, VirtualRegister},
        },
...
/// Callee-saved registers handed out by the allocator; frame lowering
/// saves and restores them in every function.
const ALLOCATABLE_GPRS: [Register; 4] =
    [registers::RBX, registers::R12, registers::R13, registers::R14];
/// Scratch registers for spill reloads/stores around one instruction.
/// r10/r11 are caller-saved; r15 is callee-saved and saved by frame lowering.
const SPILL_SCRATCH_GPRS: [Register; 3] = [registers::R10, registers::R11, registers::R15];
const SPILL_SLOT_BYTES: u64 = 8;

pub struct X86_64RegisterAllocatePass;

impl Pass for X86_64RegisterAllocatePass {
    fn name(&self) -> &str {
    ...
    block_insts
        ...
        .map(|(index, insts)| {
            ...
            for op in insts {
                ...
                match opcode {
                    x86_64_ops::JmpOp::OPCODE => {
                        has_unconditional_branch = true;
                        if let Some(target) = x86_64_ops::target(ctx, *op)
                            .and_then(|target| block_index.get(&target))
                        {
                            successors.insert(*target);
                        }
                    }
                    x86_64_ops::JccOp::OPCODE | x86_64_ops::TestJnzOp::OPCODE => {
                        if let Some(target) = x86_64_ops::target(ctx, *op)
                            .and_then(|target| block_index.get(&target))
                        {
                            successors.insert(*target);
                    ...
                    x86_64_ops::RetOp::OPCODE | x86_64_ops::Ud2Op::OPCODE => terminates = true,
                    _ => {}
                }
            }
...
fn values_live_across_calls(
    ...
    let call_indexes: BTreeSet<_> = insts
        ...
        .filter_map(|(index, op)| {
            matches!(
                x86_64_ops::opcode(ctx, *op),
                Some(x86_64_ops::CallOp::OPCODE) | Some(x86_64_ops::CallRegOp::OPCODE)
            )
            .then_some(index)
        })
        .collect();
        ...
        for (key, vreg) in use_operands {
            match assignments.get(&vreg) {
                Some(Allocation::Phys(phys_index)) => {
                    x86_64_ops::rewrite_register_operand(
                        ctx,
                        *op,
                        key,
                ...
                Some(Allocation::Spill(slot)) => {
                    ...
                    x86_64_ops::ldr_sp_offset(
                        ctx,
                        scratch,
                        spill_base_offset + slot * SPILL_SLOT_BYTES,
                    )
                    .insert_before(ctx, *op);
                    x86_64_ops::rewrite_register_operand(ctx, *op, key, scratch);
...
fn virtual_operands_with_kind(
    ...
    x86_64_ops::register_operands(ctx, inst)
        .into_iter()
        .filter_map(|operand| {
            (operand.kind == kind)
                .then_some(operand)
...
mod tests {
    ...
    fn linear_scan_reuses_expired_registers() {
        ...
        x86_64_ops::mov_imm(&mut ctx, Register::virtual_gpr(0), 1).insert_at_back(entry, &ctx);
        x86_64_ops::mov(&mut ctx, registers::RAX, Register::virtual_gpr(0)).insert_at_back(entry, &ctx);
        x86_64_ops::mov_imm(&mut ctx, Register::virtual_gpr(1), 2).insert_at_back(entry, &ctx);
        x86_64_ops::mov(&mut ctx, registers::RCX, Register::virtual_gpr(1)).insert_at_back(entry, &ctx);
    ...
    fn spills_virtual_register_live_across_call() {
        ...
        let entry = func.entry_block(&ctx);
        x86_64_ops::mov_imm(&mut ctx, Register::virtual_gpr(0), 1).insert_at_back(entry, &ctx);
        x86_64_ops::call(&mut ctx, "callee".try_into().unwrap()).insert_at_back(entry, &ctx);
        x86_64_ops::mov(&mut ctx, registers::RAX, Register::virtual_gpr(0)).insert_at_back(entry, &ctx);
        ...
        let opcodes: Vec<_> = entry
            .deref(&ctx)
            .iter(&ctx)
            .filter_map(|op| x86_64_ops::opcode(&ctx, op))
            .collect();
        assert!(
            opcodes
                .iter()
                .any(|opcode| *opcode == x86_64_ops::StrSpOffsetOp::OPCODE)
        );
        assert!(
            opcodes
                .iter()
                .any(|opcode| *opcode == x86_64_ops::LdrSpOffsetOp::OPCODE)
        );
    }

    #[test]
    fn stores_spilled_binary_definition() {
        // vr0 lives across the call, so it is force-spilled; the add that
        // reads and redefines it needs reloads for its uses and a store of
        // its def.
        let mut ctx = context();
        let func = func(&mut ctx);
        let entry = func.entry_block(&ctx);
        ...
        let add_index = insts
            .iter()
            .position(|inst| x86_64_ops::opcode(&ctx, *inst) == Some(x86_64_ops::AddOp::OPCODE))
            .unwrap();

        assert_eq!(
            x86_64_ops::opcode(&ctx, insts[add_index - 1]),
            Some(x86_64_ops::LdrSpOffsetOp::OPCODE)
        );
        assert_eq!(
            x86_64_ops::opcode(&ctx, insts[add_index + 1]),
            Some(x86_64_ops::StrSpOffsetOp::OPCODE)
        );
    ...
    fn spills_when_register_pressure_exceeds_available_registers() {
        ...
        let entry = func.entry_block(&ctx);
        for index in 0..8u32 {
            x86_64_ops::mov_imm(&mut ctx, Register::virtual_gpr(index), index as u64)
                .insert_at_back(entry, &ctx);
        }
        for index in 0..8u32 {
            x86_64_ops::mov(&mut ctx, Register::gpr(index as u8), Register::virtual_gpr(index))
                .insert_at_back(entry, &ctx);
        }
