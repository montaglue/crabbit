#!/usr/bin/env bash
set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
ROOT_DIR="$(cd "${SCRIPT_DIR}/../../../.." && pwd)"

case "$(uname -s)" in
  Darwin)
    BACKEND="${ROOT_DIR}/target/debug/libcrabbit_rust.dylib"
    ;;
  Linux)
    BACKEND="${ROOT_DIR}/target/debug/libcrabbit_rust.so"
    ;;
  MINGW*|MSYS*|CYGWIN*)
    BACKEND="${ROOT_DIR}/target/debug/crabbit_rust.dll"
    ;;
  *)
    echo "unsupported host OS: $(uname -s)" >&2
    exit 1
    ;;
esac

cargo build --manifest-path "${ROOT_DIR}/Cargo.toml" -p crabbit-rust

TARGET_DIR="${ROOT_DIR}/target/crabbit-backend-tests-structs-aarch64"
rm -rf "${TARGET_DIR}"

CARGO_TARGET_DIR="${TARGET_DIR}" \
cargo rustc --manifest-path "${SCRIPT_DIR}/Cargo.toml" --bin structs-aarch64 -- \
  "-Zcodegen-backend=${BACKEND}" \
  -Coverflow-checks=off

"${TARGET_DIR}/debug/structs-aarch64"
