...
TARGET_DIR="${ROOT_DIR}/target/crabbit-backend-tests-sudoku-aarch64"
rm -rf "${TARGET_DIR}"

CARGO_TARGET_DIR="${TARGET_DIR}" \
cargo rustc --manifest-path "${SCRIPT_DIR}/Cargo.toml" --bin sudoku-aarch64 -- \
  "-Zcodegen-backend=${BACKEND}" \
  -Coverflow-checks=off

"${TARGET_DIR}/debug/sudoku-aarch64"
